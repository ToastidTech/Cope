(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .cope-lead-card {
        ...
        overflow-y: auto;
        background: #10101e;
        border: 1px solid rgba(184,159,216,.35);
        border-radius: 24px;
        padding: 22px;
        box-shadow: 0 24px 80px rgba(0,0,0,.55);
        animation: copeLeadUp .25s ease-out;
      }
      @keyframes copeLeadUp {
      ...
      .cope-lead-card input,
      .cope-lead-card textarea {
        width: 100%;
      ...
      .cope-lead-card input:focus,
      .cope-lead-card textarea:focus {
        border-color: #b89fd8;
        box-shadow: 0 0 0 2px rgba(184,159,216,.10);
      }
      .cope-lead-card textarea {
        min-height: 100px;
        resize: vertical;
      ...
      @media (min-width: 700px) {
        #copeLeadOverlay { align-items: center; }
      }
    `;
  ...
  function injectMarkup() {
    ...
    overlay.innerHTML = `
      <div class="cope-lead-card" role="dialog" aria-modal="true" aria-labelledby="copeLeadTitle">
        ...
        <p>We'd love to hear from you. Leave your name, email, and anything you'd like to share with the Cope team.</p>
        <form id="copeLeadForm" novalidate>
          ...
          <div class="cope-lead-actions">
            ...
            <button type="submit" class="cope-lead-submit" id="copeLeadSubmit">Send</button>
          </div>
  ...
  function init() {
    ...
    let mobileBackgroundTimer = null;

    // Desktop: show when the pointer leaves toward the browser controls.
    document.addEventListener('mouseout', (event) => {
      if (event.clientY <= 4 && event.relatedTarget === null) showPrompt();
    });

    // Mobile/tablet: show when the user returns to Cope after backgrounding it.
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'hidden') {
        clearTimeout(mobileBackgroundTimer);
        mobileBackgroundTimer = setTimeout(() => {
          if (document.visibilityState === 'visible') showPrompt();
        }, 350);
      }
    });

    // Reliable fallback for devices/browsers that do not fire exit-intent events.
    // The prompt only appears once per session and can be dismissed with Not now.
